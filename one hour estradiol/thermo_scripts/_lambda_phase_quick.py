...
def W_of(p, i, t):
    bK, gK, wK, pK, bL, gL, wL, pL, phi, lam, rho, A_T, dphi = p
...
P = np.column_stack([
    ...
    rng.uniform(0.5, 6.0, N), rng.uniform(-12, 12, N)])
...
for k, i in enumerate(alive[np.argsort(vals[alive])][:55]):
    z = P[i].copy()
    for _ in range(4):
...
pr(f'  phase={phi:.2f} h   *** dphi (E2 clock phase shift) = {dphi:+.2f} h ***   A_T={A_T:.2f} C')
...
np.save('best_lambda_phase_q.npy', p)
...
fig.tight_layout(); fig.savefig('fig_lambda_phase_q.png', dpi=130, bbox_inches='tight')
pr('wrote fig_lambda_phase_q.png, best_lambda_phase_q.npy')
